...
<?php
    ...
    <?php
        include('head.php');
    ?>
    <title><?php echo $_GET['q']; ?>&bull; <?php prl("क्राया परिणाम", "Kraya Search"); ?></title>
    <?php
        include('header.php');
        echo "\n";
?>
...
<br>
    ...
    include('cred.php');
    $con = mysqli_connect($server, $user, $pass, $db);
    $sql = "SELECT * FROM `products` WHERE `keyword` LIKE '%".$_GET['q']."%' AND ".$selshop." order by `id` desc";
    if($result = mysqli_query($con, $sql)){
        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_array($result)){
                ?>
            ...
            echo "<img src='assets/bee.png' style='width:60%'><br>  <span class=error>"; prl("कुछ नहीं मिला", "No Product Found."); echo "</span>";
        }
    } else{
